...
export async function GET(request: Request) {
  try {
    ...
    if (url.searchParams.get("catalog") === "guardians") {
      return ok(await listGuardiansForStudentForm(session.schoolId));
    }

    return ok(await listStudents(session.schoolId));
  } catch (error) {
    return fail(error);
  }
}
